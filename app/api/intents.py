...
from app.services.intent_service import get_confidence_settings, save_confidence_settings
...
async def update_confidence_config(
    config: ConfidenceSettingsUpdate,
...
async def list_intents(
    ...
    result = await db.execute(
        select(Intent).offset(skip).limit(limit)
    )
    ...
    for intent in intents:
        ...
        intent_responses.append(IntentResponse(
            id=intent.id,
            name=intent.name,
