...
def _parse_pdf(file_path: str, extract_tables: bool = True) -> str:
    ...
    try:
            ...
            try:
                ...
                if extract_tables:
                    try:
                        ...
                        logger.warning(
                            f"Table extraction failed on page {page_num + 1}: {table_err}"
                        )
        ...
        logger.info(
            f"Parsed PDF: {file_path}, {len(doc)} pages, {tables_found} tables found, {len(text)} chars"
        )

        if not text.strip():
            logger.warning(
                f"PDF extracted no text, trying fallback parser: {file_path}"
            )
...
def _parse_pdf_fallback(file_path: str) -> str:
    ...
    try:
        ...
        logger.info(
            f"Fallback parsed PDF: {file_path}, {len(pages)} pages, {len(text)} chars"
        )
        return text
    except Exception as e:
...
def _extract_tables_from_page(page, page_num: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Check if extract method exists (newer pymupdf versions)
        if hasattr(tabler, "extract"):
            table_bboxes = tabler.extract()
        elif hasattr(tabler, "extract_table"):
            # Older versions use extract_table method
            table_bboxes = []
            for table in tabler:
                try:
                    table_data = table.extract_table()
                    if table_data:
                        table_bboxes.append(table_data)
                except:
                    continue
        else:
            return tables
        ...
        for table_data in table_bboxes:
            if table_data and isinstance(table_data, (list, tuple)):
                rows = table_data
                if rows and len(rows) >= 2:
                    table_dict = {
                        "headers": [
                            str(cell).strip() if cell else "" for cell in rows[0]
                        ],
                        "rows": [
                    ...
                    if _is_meaningful_table(table_dict):
                        ...
                        logger.debug(
                            f"Page {page_num + 1}: Found table with {len(rows)} rows"
                        )
    except Exception as e:
...
async def format_tables_with_llm_async(
    tables: List[Dict], document_context: str = ""
) -> str:
    """
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run, format_tables_with_llm_async(tables, document_context)
                )
