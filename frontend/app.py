...
# ============== Frontend Integration Page ==============
    ...
    if error:
        ...
        # WhatsApp and Teams sections commented out - only Feishu and Telegram active
        ...
        st.markdown(
            """
        <div style="background: #E8F5E9; padding: 16px; border-radius: 12px; margin-bottom: 16px; border-left: 4px solid #2AABEE;">
            <h3 style="color: #2AABEE; margin: 0;">Telegram Bot</h3>
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        with col1:
            ...
            if telegram_status.get("is_configured"):
            ...
            if telegram_status.get("is_configured"):
                if st.button(
                    ...
                    if err:
                        ...
                        st.error(
                            f"❌ {result.get('error', '测试失败') if result else '测试失败'}"
                        )
            else:
                st.info("请先配置 Telegram 凭据")

        with st.expander("⚙️ 配置 Telegram Bot"):
            telegram_token = st.text_input(
                "Bot Token",
                type="password",
                key="telegram_token",
                placeholder="8703736061:AAHPhY...",
            )

            if st.button("💾 保存 Telegram 凭据", key="btn_save_telegram"):
                if telegram_token:
                    result, err = api_request(
                        "PUT",
                        "/api/credentials/telegram",
                        json={"credentials": {"bot_token": telegram_token}},
                    )
                    if err:
                        st.error(err)
                    else:
                        st.success("Telegram 凭据保存成功")
                        st.rerun()
                else:
